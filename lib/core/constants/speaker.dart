import 'package:club_92/core/constants/room_list.dart';
import 'package:club_92/models/speaker_modal.dart';

List<Speaker> listOfSpeakers = [
  Speaker(
    name: 'Danial',
    username: '@daniel',
    profileImage:
        'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRjOOQLYeoZtOLftSG_sqMn0EiqyX4t9-lwAuhOtit5DtPiefzbW6-3eEcSTvGPmh-VBb8&usqp=CAU',
    followers: '24.9k',
    following: '808',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Daniel',
    youtubeHandle: 'Daniel',
    linkedInHandle: 'Daniel',
    instaHandle: 'Daniel',
    memberOf: listOfRooms,
  ),
  Speaker(
      name: 'Rubic',
      username: '@rubic',
      profileImage:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRny6M7SVLugIiTJGIYPcr744JSqVf5oPe1Vg&usqp=CAU",
      followers: '12.3k',
      following: '205',
      about:
          'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
      twitterHandle: 'Rubic',
      youtubeHandle: 'Rubic',
      linkedInHandle: 'Rubic',
      instaHandle: 'Rubic',
      memberOf: listOfRooms),
  Speaker(
    name: 'Ailseen',
    username: '@Ailseen',
    profileImage:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQzQBQuIz76W8hvZt8qxOvgCCt0mzKMd0wdzp8w9Jpr0wi1Yx3tbbj49uajKcoGZlfsrgc&usqp=CAU",
    followers: '22.3k',
    following: '405',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Ailseen',
    youtubeHandle: 'Ailseen',
    linkedInHandle: 'Ailseen',
    instaHandle: 'Ailseen',
    memberOf: listOfRooms,
  ),
  Speaker(
    name: 'Sammy',
    username: '@sammy',
    profileImage:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSEH14u7rKYD9aCAr-qRwTjpnXljCPuy4xbQSkW4HWJtCFReJNpt0-3ZW3MQyiyaIWoYyI&usqp=CAU",
    followers: '32.3k',
    following: '805',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Sammy',
    youtubeHandle: 'Sammy',
    linkedInHandle: 'Sammy',
    instaHandle: 'Sammy',
    memberOf: listOfRooms,
  ),
  Speaker(
    name: 'Monalisa',
    username: '@monalisa',
    profileImage:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRoaxcazjLKPaP2_xIYVKQV2sisCxMw9gCqo8w9NwWc6Y1xfFZfOxXmcCqiDjH2BvvHTNI&usqp=CAU",
    followers: '17.3k',
    following: '205',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Monalisa',
    youtubeHandle: 'Monalisa',
    linkedInHandle: 'Monalisa',
    instaHandle: 'Monalisa',
    memberOf: listOfRooms,
  ),
  Speaker(
    name: 'Derrick',
    username: '@derrick',
    profileImage:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRbIhi9l4npCGPNWMAc6szDbxp75kjB3c0R5w&usqp=CAU",
    followers: '18.3k',
    following: '209',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Derrick',
    youtubeHandle: 'Derrick',
    linkedInHandle: 'Derrick',
    instaHandle: 'Derrick',
    memberOf: listOfRooms,
  ),
  Speaker(
    name: 'Glenn',
    username: '@glenn',
    profileImage:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTqZZYSsnncqDhroX4Ud9rgHCxpDeyLSN5PdG71BuDAk-ulL4CQCFtjL4lKVH26UIW9EOo&usqp=CAU",
    followers: '9.3k',
    following: '102',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Glenn',
    youtubeHandle: 'Glenn',
    linkedInHandle: 'Glenn',
    instaHandle: 'Glenn',
    memberOf: listOfRooms,
  ),
  Speaker(
    name: 'Gilbert',
    username: '@gilbert',
    profileImage:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQuk0iWC7lboODUPc6ZzwMxM04ZwcfxN5MOMg4A97bmQNLBUcsvRBIMkAYuS5tcuCXLGHc&usqp=CAU",
    followers: '5.3k',
    following: '85',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Gilbert',
    youtubeHandle: 'Gilbert',
    linkedInHandle: 'Gilbert',
    instaHandle: 'Gilbert',
    memberOf: listOfRooms,
  ),
  Speaker(
    name: 'Marian',
    username: '@marian',
    profileImage:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS_-fMp-MUmqgRuvC5xpKB7_7-G26NbYOkDAshSW4klCnkkxJ65MQCLx3ZJsUVE0ynbc2Y&usqp=CAU",
    followers: '13.3k',
    following: '236',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Marian',
    youtubeHandle: 'Marian',
    linkedInHandle: 'Marian',
    instaHandle: 'Marian',
    memberOf: listOfRooms,
  ),
  Speaker(
    name: 'Dave',
    username: '@dave',
    profileImage:
        "https://static.vecteezy.com/system/resources/thumbnails/020/926/555/small/young-man-portrait-photo.jpg",
    followers: '15.3k',
    following: '250',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Dave',
    youtubeHandle: 'Dave',
    linkedInHandle: 'Dave',
    instaHandle: 'Dave',
    memberOf: listOfRooms,
  ),
  Speaker(
    name: 'Sammy',
    username: '@sammy',
    profileImage:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSEH14u7rKYD9aCAr-qRwTjpnXljCPuy4xbQSkW4HWJtCFReJNpt0-3ZW3MQyiyaIWoYyI&usqp=CAU",
    followers: '32.3k',
    following: '805',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Sammy',
    youtubeHandle: 'Sammy',
    linkedInHandle: 'Sammy',
    instaHandle: 'Sammy',
    memberOf: listOfRooms,
  ),
  Speaker(
      name: 'Danial',
      username: '@Daniel',
      profileImage:
          'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRjOOQLYeoZtOLftSG_sqMn0EiqyX4t9-lwAuhOtit5DtPiefzbW6-3eEcSTvGPmh-VBb8&usqp=CAU',
      followers: '24.9k',
      following: '808',
      about:
          'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
      twitterHandle: 'Daniel',
      youtubeHandle: 'Daniel',
      linkedInHandle: 'Daniel',
      instaHandle: 'Daniel',
      memberOf: listOfRooms),
  Speaker(
      name: 'Rubic',
      username: '@rubic',
      profileImage:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRny6M7SVLugIiTJGIYPcr744JSqVf5oPe1Vg&usqp=CAU",
      followers: '12.3k',
      following: '205',
      about:
          'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
      twitterHandle: 'Rubic',
      youtubeHandle: 'Rubic',
      linkedInHandle: 'Rubic',
      instaHandle: 'Rubic',
      memberOf: listOfRooms),
  Speaker(
    name: 'Ailseen',
    username: '@ailseen',
    profileImage:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQzQBQuIz76W8hvZt8qxOvgCCt0mzKMd0wdzp8w9Jpr0wi1Yx3tbbj49uajKcoGZlfsrgc&usqp=CAU",
    followers: '22.3k',
    following: '405',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Ailseen',
    youtubeHandle: 'Ailseen',
    linkedInHandle: 'Ailseen',
    instaHandle: 'Ailseen',
    memberOf: listOfRooms,
  ),
  Speaker(
    name: 'Monalisa',
    username: '@monalisa',
    profileImage:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRoaxcazjLKPaP2_xIYVKQV2sisCxMw9gCqo8w9NwWc6Y1xfFZfOxXmcCqiDjH2BvvHTNI&usqp=CAU",
    followers: '17.3k',
    following: '205',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Monalisa',
    youtubeHandle: 'Monalisa',
    linkedInHandle: 'Monalisa',
    instaHandle: 'Monalisa',
    memberOf: listOfRooms,
  ),
  Speaker(
    name: 'Derrick',
    username: '@derrick',
    profileImage:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRbIhi9l4npCGPNWMAc6szDbxp75kjB3c0R5w&usqp=CAU",
    followers: '18.3k',
    following: '209',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Derrick',
    youtubeHandle: 'Derrick',
    linkedInHandle: 'Derrick',
    instaHandle: 'Derrick',
    memberOf: listOfRooms,
  ),
  Speaker(
    name: 'Glenn',
    username: '@glenn',
    profileImage:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTqZZYSsnncqDhroX4Ud9rgHCxpDeyLSN5PdG71BuDAk-ulL4CQCFtjL4lKVH26UIW9EOo&usqp=CAU",
    followers: '9.3k',
    following: '102',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Glenn',
    youtubeHandle: 'Glenn',
    linkedInHandle: 'Glenn',
    instaHandle: 'Glenn',
    memberOf: listOfRooms,
  ),
  Speaker(
    name: 'Gilbert',
    username: '@gilbert',
    profileImage:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQuk0iWC7lboODUPc6ZzwMxM04ZwcfxN5MOMg4A97bmQNLBUcsvRBIMkAYuS5tcuCXLGHc&usqp=CAU",
    followers: '5.3k',
    following: '85',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Gilbert',
    youtubeHandle: 'Gilbert',
    linkedInHandle: 'Gilbert',
    instaHandle: 'Gilbert',
    memberOf: listOfRooms,
  ),
  Speaker(
    name: 'Marian',
    username: '@marian',
    profileImage:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS_-fMp-MUmqgRuvC5xpKB7_7-G26NbYOkDAshSW4klCnkkxJ65MQCLx3ZJsUVE0ynbc2Y&usqp=CAU",
    followers: '13.3k',
    following: '236',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Marian',
    youtubeHandle: 'Marian',
    linkedInHandle: 'Marian',
    instaHandle: 'Marian',
    memberOf: listOfRooms,
  ),
  Speaker(
    name: 'Dave',
    username: '@dave',
    profileImage:
        "https://static.vecteezy.com/system/resources/thumbnails/020/926/555/small/young-man-portrait-photo.jpg",
    followers: '15.3k',
    following: '250',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Dave',
    youtubeHandle: 'Dave',
    linkedInHandle: 'Dave',
    instaHandle: 'Dave',
    memberOf: listOfRooms,
  ),
  Speaker(
    name: 'Sammy',
    username: '@sammy',
    profileImage:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSEH14u7rKYD9aCAr-qRwTjpnXljCPuy4xbQSkW4HWJtCFReJNpt0-3ZW3MQyiyaIWoYyI&usqp=CAU",
    followers: '32.3k',
    following: '805',
    about:
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
    twitterHandle: 'Sammy',
    youtubeHandle: 'Sammy',
    linkedInHandle: 'Sammy',
    instaHandle: 'Sammy',
    memberOf: listOfRooms,
  ),
  Speaker(
      name: 'Danial',
      username: '@Daniel',
      profileImage:
          'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRjOOQLYeoZtOLftSG_sqMn0EiqyX4t9-lwAuhOtit5DtPiefzbW6-3eEcSTvGPmh-VBb8&usqp=CAU',
      followers: '24.9k',
      following: '808',
      about:
          'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
      twitterHandle: 'Daniel',
      youtubeHandle: 'Daniel',
      linkedInHandle: 'Daniel',
      instaHandle: 'Daniel',
      memberOf: listOfRooms),
  Speaker(
      name: 'Rubic',
      username: '@rubic',
      profileImage:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRny6M7SVLugIiTJGIYPcr744JSqVf5oPe1Vg&usqp=CAU",
      followers: '12.3k',
      following: '205',
      about:
          'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed ac justo nec risus varius pellentesque. Nullam euismod, ligula vel eleifend interdum, justo lacus feugiat ex, ut ultrices ligula lectus vel ex. Quisque convallis augue id ante scelerisque, nec vulputate justo malesuada.',
      twitterHandle: 'Rubic',
      youtubeHandle: 'Rubic',
      linkedInHandle: 'Rubic',
      instaHandle: 'Rubic',
      memberOf: listOfRooms),
];
